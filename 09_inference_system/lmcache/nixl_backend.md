...
为了适应不同的底层存储特性，`NixlStorageBackend` 采用了 **Static**（静态）与 **Dynamic**（动态）两种派生实现：**Static 模式通过预分配资源池实现极低开销的固定容量存储，Dynamic 模式则通过按需创建对象适配无限容量的云端存储场景。**

### 1.1 类继承结构

**代码位置**: [`lmcache/v1/storage_backend/nixl_storage_backend.py`](../lmcache/v1/storage_backend/nixl_storage_backend.py)

- **`NixlStorageBackend` (Base)**: 定义了通用的 `put`/`get` 接口、锁机制 (`key_lock`)、进度追踪 (`progress_set`) 以及与 `NixlAgent` 的交互逻辑。
- **`NixlStaticStorageBackend`**:
  - **适用场景**: 文件系统类后端（POSIX, GDS, GDS_MT, HF3FS）及**静态对象存储**（OBJ）。
  - **特点**: 使用预分配的资源池（Pool），容量固定。对于对象存储，它会预先生成一组 Key。
  - **索引**: 维护本地内存索引 (`self.key_dict`)，类似于 `LocalDiskBackend`。
- **`NixlDynamicStorageBackend`**:
  - **适用场景**: 动态对象存储后端（OBJ / S3）。
  - **特点**: 无资源池限制，容量无限，按需创建对象 Key。
  - **索引**: 依赖 **Presence Cache** (如 Bloom Filter) 进行快速存在性检测，减少远程 API 调用。
...
| **容量限制** | **受限**。受 `nixl_pool_size` 限制，满时需驱逐 (Evict)。   | **无限**。仅受限于底层存储系统的容量。                 |
...
extra_config:
  enable_nixl_storage: true
  nixl_backend: OBJ
  nixl_pool_size: 0 # 0 = Dynamic Mode
