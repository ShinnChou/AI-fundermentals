...
- **1g.10gb**：1/7 GPU + 10GB 显存（A100 80GB）
- **2g.20gb**：2/7 GPU + 20GB 显存（A100 80GB）
- **3g.40gb**：3/7 GPU + 40GB 显存（A100 80GB）
- **7g.80gb**：完整 GPU + 80GB 显存（A100 80GB）

**性能效果：**

- **并发能力**：最多可创建7个独立实例，并发能力提升7倍
- **延迟优化**：硬件级隔离减少调度开销，平均延迟降低30%
- **资源利用率**：支持细粒度资源分配，GPU利用率提升40-60%
...
- **内存虚拟化优化**：HBM3的高带宽特性使内存虚拟化开销降低至2%以下
