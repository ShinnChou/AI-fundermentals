...
# 用法: ./start_multi_node_test.sh <node_rank> <master_addr> [options]
#
# 示例:
#   # 主节点 (node_rank=0)
...
# 脚本信息
SCRIPT_NAME="start_multi_node_test.sh"
VERSION="1.0.0"
...
check_environment() {
    ...
    # 检查 nccl_test.sh 脚本
    if [ ! -f "./nccl_test.sh" ]; then
        log_error "找不到 nccl_test.sh 脚本"
        log_info "请确保在正确的目录下运行此脚本"
        exit 1
    fi
    log_success "找到 nccl_test.sh 脚本"
    
    # 检查脚本权限
    if [ ! -x "./nccl_test.sh" ]; then
        log_warning "nccl_test.sh 脚本没有执行权限，正在添加..."
        chmod +x "./nccl_test.sh"
        log_success "已添加执行权限"
    fi
...
start_test() {
    ...
    log_info ""
    log_info "启动测试命令:"
    local test_cmd="./nccl_test.sh -m --master-addr $MASTER_ADDR --master-port $MASTER_PORT --network $NETWORK -s $TEST_SIZE -t $TEST_DURATION"
    log_info "$test_cmd"
