...
**OpenViking** 是一个专为 AI Agent 设计的**上下文数据库**，由字节跳动开源。它解决的是 AI Agent 开发中普遍存在的上下文管理难题。
...
> **为什么需要三层？** 想象你要了解一本书：L0 相当于书名和目录标题（快速判断是否相关），L1 相当于章节摘要（了解大概内容），L2 相当于完整正文（深入阅读）。你不会一上来就读整本书，而是先看目录，再选感兴趣的章节精读。
...
> **解析过程**：当你导入一个项目文档或代码仓库时，OpenViking 会自动解析文件内容、提取关键信息、生成 L0/L1 层摘要，并创建向量索引用于后续检索。
...
## 九、VikingBot - Agent 应用层

[VikingBot](https://github.com/volcengine/OpenViking/tree/main/bot) 是基于 [Nanobot](https://github.com/HKUDS/nanobot) 框架构建的 AI 助手应用，深度集成 OpenViking 作为上下文管理后端。
