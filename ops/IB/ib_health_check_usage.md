...
- **MTU 设置优化**：推荐从标准 2044 调整为 Jumbo Frame 65520
...
优化命令:
  # 创建优化配置文件
  sudo tee /etc/sysctl.d/99-ib-network.conf << EOF
# InfiniBand 网络优化参数
...
EOF
  # 应用配置
  sudo sysctl -p /etc/sysctl.d/99-ib-network.conf
```
