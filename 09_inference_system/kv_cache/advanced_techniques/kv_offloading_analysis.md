...
1. **OffloadingSpec**: 定义卸载策略的基类，位于 [`vllm/v1/kv_offload/spec.py`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/kv_offload/spec.py)。它支持配置 `offloaded_block_size`（通常大于 GPU 的 `block_size` 以优化传输带宽），并负责实例化 Manager 和 Handler。
...
2. **OffloadingManager (Scheduler 侧)**:
   ...
   - 提供了多种淘汰策略的实现：
     - **LRU (Least Recently Used)**: 默认策略，由 [`LRUOffloadingManager`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/kv_offload/lru_manager.py#L16) 实现。
     ...
     - **ARC (Adaptive Replacement Cache)**: 自适应策略，由 [`ARCOffloadingManager`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/kv_offload/arc_manager.py#L16) 实现，能够在 Recency (最近访问) 和 Frequency (频繁访问) 之间动态平衡。
...
3. **OffloadingWorker (Worker 侧)**:
   - 驻留在 GPU Worker 进程中，由 [`OffloadingWorker`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/kv_offload/worker/worker.py#L80) 类管理。
   ...
   - 通过具体的 [`OffloadingHandler`](https://github.com/vllm-project/vllm/blob/main/vllm/v1/kv_offload/worker/worker.py#L18) 执行实际的数据传输。
...
1. **Store (卸载)**: 当一个请求生成新的 KV Block，或者被抢占时，Scheduler 标记这些 Block 需要卸载。这部分逻辑在 [`OffloadingConnectorScheduler._get_reqs_to_store`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/offloading_connector.py#L276) 中实现：
...
2. **Load (加载)**: 当请求被重新调度时，Scheduler 识别出该请求在 CPU 中有缓存的 Block，并生成加载指令。这部分逻辑在 [`OffloadingConnectorScheduler.update_state_after_alloc`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/offloading_connector.py#L234) 中处理：
...
在 vLLM 中，**LMCacheConnector** 充当了物理内存视图与 LMCache 逻辑视图之间的智能适配器。其核心实现类 [`LMCacheConnectorV1`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/lmcache_connector.py#L72) 采用适配器模式，根据配置动态加载底层的 LMCache 实现（通常通过 `vllm_v1_adapter` 进行桥接）。
...
- **LMCacheEngine**: 核心引擎，负责管理底层的存储后端（Local CPU, Disk, Redis, P2P 等）。在 vLLM 适配层中，通过 [`LMCacheConnectorV1Impl`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/lmcache_integration/vllm_v1_adapter.py#L210) 进行封装。
...
- **RequestTracker**: 跟踪每个请求的状态，记录请求对应的 Token 序列以及在 vLLM 中分配的 Block ID。源码可见 [`RequestTracker`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/lmcache_integration/vllm_v1_adapter.py#L121)。
...
- **Layerwise Pipelining (层级流水线)**: 利用 Transformer 的层级结构，在计算第 $i$ 层时，异步预取第 $i+1$ 层的 KV 数据。这主要通过 [`wait_for_layer_load`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/lmcache_connector.py#L143) 接口实现，允许 Attention Layer 阻塞等待特定层的加载，实现 I/O 与计算的完美重叠。
...
- **LMCBlender (混合器)**: 支持将部分 KV Cache 放在 GPU，部分放在远程，通过 Blender 机制在推理时动态合并。该功能通过适配层中的 [`LMCBlenderBuilder`](https://github.com/vllm-project/vllm/blob/main/vllm/distributed/kv_transfer/kv_connector/v1/lmcache_integration/vllm_v1_adapter.py#L17) 进行集成，在启用时负责初始化混合引擎。
